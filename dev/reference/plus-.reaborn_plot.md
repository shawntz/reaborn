...
[`catplot()`](https://reaborn.org/dev/reference/catplot.md) uses) and
then apply the user's facet – so each panel shows, and for aggregating
kinds re-estimates, its own subset instead of one shared full-data
summary (#73).
