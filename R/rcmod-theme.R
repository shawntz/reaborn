...
axes_style <- function(style = NULL, rc = NULL) {
  ...
  if (is.list(style)) {
    return(utils::modifyList(style, rc %||% list()))
  }
  if (!style %in% names(.RB_STYLES)) {
    stop(sprintf(
      "style must be one of %s",
      paste(names(.RB_STYLES), collapse = ", ")
    ))
  }
...
theme_seaborn <- function(
  ...
  ticks_elem <- if (st$ticks) {
    ggplot2::element_line(
      colour = text_col,
      linewidth = .rb_lw(ctx$xtick.major.width)
    )
  } else {
    ggplot2::element_blank()
  }
  ...
  base +
    ggplot2::theme(
      ...
      axis.title = ggplot2::element_text(
        size = ctx$axes.labelsize,
        colour = text_col
      ),
      axis.text = ggplot2::element_text(
        size = ctx$xtick.labelsize,
        colour = text_col
      ),
      plot.title = ggplot2::element_text(
        size = ctx$axes.titlesize,
        colour = text_col
      ),
      legend.text = ggplot2::element_text(
        size = ctx$legend.fontsize,
        colour = text_col
      ),
      legend.title = ggplot2::element_text(
        size = ctx$legend.title_fontsize,
        colour = text_col
      ),
  ...
  options(
    ...
    ggplot2.continuous.colour = function(...) {
      ggplot2::scale_colour_gradientn(
        ...,
        colours = attr(color_palette("rocket", as_cmap = TRUE), "colors")
      )
    },
...
.reaborn_get <- function(key, default) {
  if (exists(key, envir = .reaborn, inherits = FALSE)) {
    get(key, envir = .reaborn)
  } else {
    default
  }
}
