...
histplot <- function(
  ...
  if (hue_present) {
    ...
    levels <- "_all"
    groups <- factor(rep("_all", length(vals)))
  ...
  ld <- if (horizontal) {
    list(x = bars$y, y = bars$xmid)
  } else {
    list(x = bars$xmid, y = bars$ymax)
  }
  p <- rb_finish_plot(
    p,
...
rb_hist_alpha <- function(fill, hue, multiple, element, kde) {
  ...
  if (hue && multiple == "layer") {
    return(if (element == "bars") 0.5 else 0.25)
  }
  if (kde) {
    return(0.5)
  }
...
kdeplot <- function(
  ...
  if (hue_present) {
    ...
    colors <- stats::setNames(
      color %||% color_palette(.reaborn_get("palette", "deep"), 1),
      lv
    )
  }
  ...
  curves <- do.call(
    ...
    lapply(seq_len(nrow(combos)), function(i) {
      ...
      if (common_norm && !cumulative) {
        est$y <- est$y * length(gx) / total
      }
      row <- data.frame(
        group = combos$.group[i],
        x = est$x,
  ...
  ld <- if (horizontal) {
    list(x = curves$y, y = curves$x)
  } else {
    list(x = curves$x, y = curves$y)
  }
  p <- rb_finish_plot(
    p,
    xlab = if (horizontal) dens_lab else v$names$val,
...
rb_kde_stack <- function(curves, multiple, levels) {
  ...
  mat <- vapply(
    ...
    function(g) {
      sub <- wide[[g]]
      stats::approx(sub$x, sub$y, grid, rule = 2)$y
    },
    numeric(length(grid))
  )
...
rb_kdeplot_bivariate <- function(
  ...
  lev <- rb_iso_proportion_levels(
    dens$z,
    if (length(levels) == 1) levels else levels,
    thresh
  )
  col <- color %||% color_palette(.reaborn_get("palette", "deep"), 1)
...
ecdfplot <- function(
  ...
  p <- rb_finish_plot(
    p,
    xlab = if (horizontal) lab else v$names$val,
...
rugplot <- function(
  ...
  if ("x" %in% names(vd)) {
    base$`__x` <- vd$x
    mapping$x <- rlang::sym("__x")
  }
  if ("y" %in% names(vd)) {
    base$`__y` <- vd$y
    mapping$y <- rlang::sym("__y")
  }
  sides <- paste0(
    if ("x" %in% names(vd)) "b" else "",
    if ("y" %in% names(vd)) "l" else ""
  )
  colors <- NULL
  if (hue_present) {
    lv <- rb_categorical_order(vd[["hue"]], hue_order)
  ...
  if (!hue_present) {
    rug_args$colour <- color_palette(.reaborn_get("palette", "deep"), 1)
  }
