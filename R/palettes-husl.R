...
.husl_max_chroma <- function(L, H) {
  ...
  for (i in seq_len(nrow(.husl_m))) {
    m1 <- .husl_m[i, 1]; m2 <- .husl_m[i, 2]; m3 <- .husl_m[i, 3]
...
.husl_xyz_to_luv <- function(triple) {
  ...
  if (X == 0 && Y == 0 && Z == 0) return(c(0, 0, 0))
  varU <- (4 * X) / (X + 15 * Y + 3 * Z)
  varV <- (9 * Y) / (X + 15 * Y + 3 * Z)
  L <- 116 * .husl_f(Y / .husl_refY) - 16
  if (L == 0) return(c(0, 0, 0))
...
.husl_luv_to_xyz <- function(triple) {
  L <- triple[1]; U <- triple[2]; V <- triple[3]
  if (L == 0) return(c(0, 0, 0))
...
.husl_lch_to_luv <- function(triple) {
  L <- triple[1]; C <- triple[2]; H <- triple[3]
  Hrad <- H * pi / 180
  c(L, cos(Hrad) * C, sin(Hrad) * C)
}

.husl_to_lch <- function(triple) {
  H <- triple[1]; S <- triple[2]; L <- triple[3]
  if (L > 99.9999999) return(c(100, 0, H))
  if (L < 0.00000001) return(c(0, 0, H))
...
.rgb_to_husl <- function(r, g, b) {
  ...
  if (L > 99.9999999) return(c(H, 0, 100))
  if (L < 0.00000001) return(c(H, 0, 0))
  mx <- .husl_max_chroma(L, H)
  S <- C / mx * 100
  c(H, S, L)
}
