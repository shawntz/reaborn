...
.rb_reg_fit <- function(x, y, grid, order = 1, logistic = FALSE, lowess = FALSE,
                        robust = FALSE, logx = FALSE) {
  if (lowess) {
...
.rb_reg_band <- function(x, y, grid, ci, n_boot, seed, ...) {
  ...
  boots <- vapply(seq_len(n_boot), function(i) {
    ...
    tryCatch(.rb_reg_fit(x[idx], y[idx], grid, ...), error = function(e) rep(NA_real_, length(grid)))
  }, numeric(length(grid)))
  edge <- (100 - ci) / 2
  data.frame(
    grid = grid,
...
regplot <- function(data = NULL, x = NULL, y = NULL, order = 1, logistic = FALSE,
                    ...
                    n_boot = 1000, seed = NULL, scatter = TRUE, fit_reg = TRUE,
                    color = NULL, marker = "o", scatter_kws = NULL, line_kws = NULL,
                    truncate = TRUE, x_jitter = NULL, y_jitter = NULL, ...) {
  ...
  if (scatter) {
    ...
    p <- p + ggplot2::geom_point(
      colour = sk$color %||% col, fill = sk$color %||% col,
      size = sk$size %||% rb_area_to_size(36), shape = 21, stroke = 0,
      alpha = sk$alpha %||% 1
    )
  }
  ...
  if (fit_reg) {
    ...
    yhat <- .rb_reg_fit(xv, yv, grid, order = order, logistic = logistic,
                        lowess = lowess, robust = robust, logx = logx)
    fit_df <- data.frame(grid = grid, yhat = yhat)
    band <- .rb_reg_band(xv, yv, grid, ci, n_boot, seed, order = order,
                         logistic = logistic, lowess = lowess, robust = robust, logx = logx)
    if (!is.null(band)) {
      p <- p + ggplot2::geom_ribbon(
        data = band, mapping = ggplot2::aes(x = .data$grid, ymin = .data$lo, ymax = .data$hi),
        inherit.aes = FALSE, fill = col, alpha = 0.15)
    }
    lk <- line_kws %||% list()
    p <- p + ggplot2::geom_line(
      data = fit_df, mapping = ggplot2::aes(x = .data$grid, y = .data$yhat),
      inherit.aes = FALSE, colour = lk$color %||% col,
...
residplot <- function(data = NULL, x = NULL, y = NULL, lowess = FALSE, order = 1,
  ...
  p <- ggplot2::ggplot(rdf, ggplot2::aes(x = .data$x, y = .data$resid)) +
    ggplot2::geom_hline(yintercept = 0, linewidth = rb_line_default_width(),
                        colour = RB_BOX_LINECOLOR) +
    ggplot2::geom_point(colour = col, fill = col, shape = 21, stroke = 0,
                        size = rb_area_to_size(36))
  if (lowess) {
    lo <- stats::lowess(rdf$x, rdf$resid, f = 2 / 3)
    p <- p + ggplot2::geom_line(data = data.frame(x = lo$x, y = lo$y),
                                mapping = ggplot2::aes(x = .data$x, y = .data$y),
                                inherit.aes = FALSE, colour = col,
...
#' @return A `reaborn_plot`.
#' @export
lmplot <- function(data = NULL, x = NULL, y = NULL, hue = NULL, col = NULL,
                   row = NULL, palette = NULL, col_wrap = NULL, height = 5,
                   aspect = 1, order = 1, logistic = FALSE, lowess = FALSE,
                   robust = FALSE, logx = FALSE, ci = 95, n_boot = 1000, seed = NULL,
                   scatter = TRUE, fit_reg = TRUE, hue_order = NULL, row_order = NULL,
                   col_order = NULL, legend = TRUE, facet_kws = NULL, ...) {
  v <- rb_assign_variables(data, x = x, y = y, hue = hue)
  df <- rb_drop_na(v$data, c("x", "y"))
  base <- if (is.data.frame(data)) data[stats::complete.cases(v$data[c("x", "y")]), , drop = FALSE] else df
  ...
  if (hue_present) {
    ...
    colors <- stats::setNames(color_palette(.reaborn_get("palette", "deep"), 1), "_all")
    groups <- "_all"
  }

  # Draw scatter + fit per hue group (so colors and fits separate).
  for (g in groups) {
    sub <- if (hue_present) base[as.character(base[[hue]]) == g, , drop = FALSE] else base
    gcol <- colors[[g]]
    if (scatter) {
      p <- p + ggplot2::geom_point(data = sub, colour = gcol, fill = gcol, shape = 21,
                                   stroke = 0, size = rb_area_to_size(36))
    }
    if (fit_reg && nrow(sub) > order + 1) {
      grid <- seq(min(sub[[x]]), max(sub[[x]]), length.out = 100)
      yhat <- .rb_reg_fit(sub[[x]], sub[[y]], grid, order = order, logistic = logistic,
                          lowess = lowess, robust = robust, logx = logx)
      band <- .rb_reg_band(sub[[x]], sub[[y]], grid, ci, n_boot, seed, order = order,
                           logistic = logistic, lowess = lowess, robust = robust, logx = logx)
      if (!is.null(band)) {
        p <- p + ggplot2::geom_ribbon(data = band, inherit.aes = FALSE,
          mapping = ggplot2::aes(x = .data$grid, ymin = .data$lo, ymax = .data$hi),
          fill = gcol, alpha = 0.15)
      }
      p <- p + ggplot2::geom_line(data = data.frame(grid = grid, yhat = yhat),
        mapping = ggplot2::aes(x = .data$grid, y = .data$yhat), inherit.aes = FALSE,
        colour = gcol, linewidth = rb_line_default_width())
    }
  }
  ...
  if (hue_present) {
    p <- p + ggplot2::geom_point(
      data = base, mapping = ggplot2::aes(x = .data[[x]], y = .data[[y]], colour = .data[[hue]]),
