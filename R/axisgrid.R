...
.rb_marginal_theme <- function(axis = "x") {
  ...
  if (axis == "x") {
    base + ggplot2::theme(axis.title.x = ggplot2::element_blank(),
                          axis.text.x = ggplot2::element_blank(),
                          axis.ticks.x = ggplot2::element_blank(),
                          axis.text.y = ggplot2::element_blank(),
                          axis.ticks.y = ggplot2::element_blank())
  } else {
    base + ggplot2::theme(axis.text = ggplot2::element_blank(),
...
#' @export
jointplot <- function(data = NULL, x = NULL, y = NULL, hue = NULL, kind = "scatter",
                      height = 6, ratio = 5, space = 0.2, color = NULL,
                      palette = NULL, ...) {
  if (!requireNamespace("patchwork", quietly = TRUE)) {
    stop("jointplot() requires the 'patchwork' package.")
  }
  main <- switch(kind,
    scatter = scatterplot(data = data, x = x, y = y, hue = hue, palette = palette,
                          legend = FALSE, ...),
  ...
  top <- mfun(data = data, x = x, hue = hue, palette = palette, legend = FALSE) +
    .rb_marginal_theme("x")
  right <- mfun(data = data, y = y, hue = hue, palette = palette, legend = FALSE) +
    .rb_marginal_theme("y")

  layout <- top + patchwork::plot_spacer() + main + right +
    patchwork::plot_layout(ncol = 2, nrow = 2, widths = c(ratio, 1),
                           heights = c(1, ratio))
...
#' @return A `reaborn_plot` (patchwork).
#' @export
pairplot <- function(data, vars = NULL, hue = NULL, kind = "scatter",
                     diag_kind = "auto", palette = NULL, height = 2.5,
                     aspect = 1, corner = FALSE, ...) {
  if (!requireNamespace("patchwork", quietly = TRUE)) {
  ...
  if (identical(diag_kind, "auto")) diag_kind <- if (is.null(hue)) "hist" else "kde"
  n <- length(vars)
  ...
  for (i in seq_len(n)) {
    for (j in seq_len(n)) {
      ...
      if (i == j) {
        pl <- if (diag_kind == "kde") {
          kdeplot(data = data, x = xj, hue = hue, palette = palette, legend = FALSE)
        } else {
          histplot(data = data, x = xj, hue = hue, palette = palette, legend = FALSE)
        }
      } else if (kind == "reg") {
        pl <- regplot(data = data, x = xj, y = yi)
      } else {
        pl <- scatterplot(data = data, x = xj, y = yi, hue = hue, palette = palette,
                          legend = FALSE)
      }
      # Only show axis titles on the outer edges.
      pl <- pl + ggplot2::theme(legend.position = "none")
      if (i != n) pl <- pl + ggplot2::theme(axis.title.x = ggplot2::element_blank())
      if (j != 1) pl <- pl + ggplot2::theme(axis.title.y = ggplot2::element_blank())
      plots[[idx]] <- pl
      idx <- idx + 1
    }
  }
...
#' @export
FacetGrid <- function(data, row = NULL, col = NULL, hue = NULL, col_wrap = NULL,
                      height = 3, aspect = 1, palette = NULL) {
  p <- ggplot2::ggplot(data)
  if (!is.null(hue)) {
    colors <- rb_categorical_colors(length(rb_categorical_order(data[[hue]])), palette)
    p <- p + ggplot2::aes(colour = .data[[hue]]) +
