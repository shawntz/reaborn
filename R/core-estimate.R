...
rb_parse_errorbar <- function(errorbar = list("ci", 95)) {
  if (is.null(errorbar)) {
    return(list(method = NULL, level = NULL))
  }
  if (is.function(errorbar)) {
    return(list(method = errorbar, level = NULL))
  }
...
rb_estimate_aggregator <- function(
  vals,
  estimator = "mean",
  errorbar = list("ci", 95),
  n_boot = 1000,
  ...
  iv <- switch(
    eb$method,
    sd = {
      h <- stats::sd(vals) * eb$level
      c(est - h, est + h)
    },
    se = {
      h <- stats::sd(vals) / sqrt(length(vals)) * eb$level
      c(est - h, est + h)
    },
