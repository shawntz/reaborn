...
rb_resolve_size <- function(
  ...
  cont_scale <- if (aes == "linewidth") {
    ggplot2::scale_linewidth_continuous
  } else {
    ggplot2::scale_size_continuous
  }
  man_scale <- if (aes == "linewidth") {
...
rb_resolve_style <- function(
  ...
  if (isTRUE(markers) || is.character(markers)) {
    ...
    out$shape_scale <- ggplot2::scale_shape_manual(
      values = stats::setNames(shp, levels),
      name = name
    )
  }
  if (isTRUE(dashes)) {
    ...
    out$linetype_scale <- ggplot2::scale_linetype_manual(
      values = stats::setNames(lty, levels),
      name = name
    )
  }
  out
