...
.rb_violin_groups <- function(s, width, gap) {
  ...
  for (fi in seq_len(nrow(fcombos))) {
    ...
    for (fv in fvars) fsel <- fsel & as.character(s$df[[fv]]) == as.character(fcombos[fi, fv])
    ...
    for (ci in seq_along(cat_levels)) {
      for (hi in seq_along(hue_levels)) {
        ...
        if (s$has_hue) sel <- sel & as.character(s$df$.hue) == hue_levels[hi]
        vals <- s$df$.val[sel]
        vals <- vals[!is.na(vals)]
        offset <- if (s$has_hue) (hi - (n_hue + 1) / 2) * (width / n_hue) else 0
        groups[[length(groups) + 1]] <- list(
          cat = lv, hue = hue_levels[hi], hue_idx = hi, pos = ci + offset,
          vals = vals, half = sub_w / 2, facets = facets)
      }
    }
  }
  groups
...
.rb_attach_facets <- function(df, g) {
  if (!is.null(g$facets)) for (fv in names(g$facets)) df[[fv]] <- g$facets[[1, fv]]
  df
}
...
violinplot <- function(data = NULL, x = NULL, y = NULL, hue = NULL, order = NULL,
  ...
  for (i in seq_along(groups)) {
    g <- groups[[i]]
    if (length(g$vals) < 2) { groups[[i]]$kde <- NULL; next }
    groups[[i]]$kde <- rb_gaussian_kde(g$vals, bw_method, bw_adjust, gridsize, cut)
  ...
  polys <- list(); inner_segs <- list(); inner_pts <- list(); pi <- 0
  for (g in groups) {
    if (is.null(g$kde)) next
    pi <- pi + 1
    dens <- g$kde$y
    scale <- switch(density_norm,
    ...
    polys[[pi]] <- .rb_attach_facets(
      data.frame(id = pi, x = px, y = py, fill = g$hue, stringsAsFactors = FALSE), g)
    ...
    if (!is.null(inner) && !isFALSE(inner)) {
      ...
      if (inner == "box") {
        inner_segs[[length(inner_segs) + 1]] <- af(data.frame(
          pos = g$pos, ylo = q[1], yhi = q[3], lw = "thick"))
        inner_segs[[length(inner_segs) + 1]] <- af(data.frame(
          pos = g$pos, ylo = lo, yhi = hi, lw = "thin"))
        inner_pts[[length(inner_pts) + 1]] <- af(data.frame(pos = g$pos, y = q[2]))
      } else if (inner == "quart") {
        for (qq in q) inner_segs[[length(inner_segs) + 1]] <- af(data.frame(
          pos = g$pos, ylo = qq, yhi = qq, lw = "quart",
          xmin = g$pos - max(w), xmax = g$pos + max(w)))
      } else if (inner == "stick") {
        for (vv in g$vals) inner_segs[[length(inner_segs) + 1]] <- af(data.frame(
          pos = g$pos, ylo = vv, yhi = vv, lw = "stick"))
      } else if (inner == "point") {
        for (vv in g$vals) inner_pts[[length(inner_pts) + 1]] <- af(data.frame(pos = g$pos, y = vv))
      }
    }
  }
  poly_df <- do.call(rbind, polys)

  flip <- function(df, xcol, ycol) if (vert) df else { tmp <- df[[xcol]]; df[[xcol]] <- df[[ycol]]; df[[ycol]] <- tmp; df }

  pmap <- if (vert) ggplot2::aes(x = .data$x, y = .data$y, group = .data$id)
          else ggplot2::aes(x = .data$y, y = .data$x, group = .data$id)
  if (s$has_hue) pmap$fill <- rlang::sym("fill")
  poly_args <- list(mapping = pmap, colour = lc, linewidth = lw)
  if (!s$has_hue) poly_args$fill <- colors
  p <- ggplot2::ggplot(poly_df) + do.call(ggplot2::geom_polygon, poly_args)

  # Inner box / quart / stick / point.
  if (length(inner_segs)) {
    seg <- do.call(rbind, lapply(inner_segs, function(d) { d$lwval <- d$lw[1]; d }))
    for (kind in unique(seg$lw)) {
      sub <- seg[seg$lw == kind, ]
      thick <- switch(kind, thick = .rb_lw(6), thin = .rb_lw(1.5), quart = .rb_lw(1.5),
                      stick = .rb_lw(1), .rb_lw(1.5))
      seg_aes <- if (vert) ggplot2::aes(x = .data$pos, xend = .data$pos, y = .data$ylo, yend = .data$yhi)
                 else ggplot2::aes(y = .data$pos, yend = .data$pos, x = .data$ylo, xend = .data$yhi)
      p <- p + ggplot2::geom_segment(data = sub, mapping = seg_aes,
                                     colour = if (kind == "stick") "white" else RB_BOX_LINECOLOR,
                                     linewidth = thick, lineend = "round")
    }
  }
  if (length(inner_pts)) {
    pts <- do.call(rbind, inner_pts)
    pt_aes <- if (vert) ggplot2::aes(x = .data$pos, y = .data$y) else ggplot2::aes(x = .data$y, y = .data$pos)
    p <- p + ggplot2::geom_point(data = pts, mapping = pt_aes, colour = "white",
                                 fill = "white", size = rb_area_to_size(9))
  }
  ...
  cat_scale <- if (vert) ggplot2::scale_x_continuous else ggplot2::scale_y_continuous
  val_scale <- if (vert) ggplot2::scale_y_continuous else ggplot2::scale_x_continuous
  p <- p + cat_scale(breaks = seq_along(s$cat_levels), labels = s$cat_levels) +
...
.rb_lv_k <- function(n, k_depth = "tukey", outlier_prop = 0.007, trust_alpha = 0.05) {
  k <- if (is.numeric(k_depth)) as.integer(k_depth)
       else switch(k_depth,
...
rb_letter_values <- function(x, k_depth = "tukey", outlier_prop = 0.007, trust_alpha = 0.05) {
  x <- x[!is.na(x)]
  n <- length(x)
  ...
  if (identical(k_depth, "full")) { percs[1] <- 0; percs[length(percs)] <- 100 }
  values <- stats::quantile(x, percs / 100, names = FALSE, type = 7)
  fliers <- x[x < min(values) | x > max(values)]
  list(k = k, levels = levels, percs = percs, values = values,
       fliers = fliers, median = stats::quantile(x, 0.5, names = FALSE, type = 7))
}
...
boxenplot <- function(data = NULL, x = NULL, y = NULL, hue = NULL, order = NULL,
                      hue_order = NULL, orient = NULL, color = NULL, palette = NULL,
                      saturation = 0.75, fill = TRUE, width = 0.8, gap = 0,
                      linewidth = NULL, linecolor = NULL, width_method = "exponential",
                      k_depth = "tukey", outlier_prop = 0.007, trust_alpha = 0.05,
                      showfliers = TRUE, legend = "auto", .facet_vars = NULL, ...) {
  s <- rb_cat_setup(data, x, y, hue, order, hue_order, orient, .facet_vars)
  vert <- s$orient == "v"
  ...
  boxes <- list(); meds <- list(); fliers <- list()
  for (g in groups) {
    if (length(g$vals) < 2) next
    maincolor <- if (s$has_hue) base_colors[[g$hue]] else base_colors[1]
    lv <- rb_letter_values(g$vals, k_depth, outlier_prop, trust_alpha)
    vals <- lv$values; nbox <- length(vals) - 1
    exponent <- lv$levels - 1 - lv$k
    rel <- switch(width_method,
      linear = lv$levels + 1,
      exponential = 2^exponent,
      area = { tails <- lv$levels < (lv$k - 1); 2^(exponent - tails) / diff(vals) },
      2^exponent)
    widths <- rel / max(rel) * (2 * g$half)
    cmap <- light_palette(maincolor, as_cmap = TRUE)
    boxcolors <- cmap(2^((exponent + 2) / 3))
    for (i in seq_len(nbox)) {
      boxes[[length(boxes) + 1]] <- .rb_attach_facets(data.frame(
        xmin = g$pos - widths[i] / 2, xmax = g$pos + widths[i] / 2,
        ymin = vals[i], ymax = vals[i + 1], fillcol = boxcolors[i],
        stringsAsFactors = FALSE), g)
    }
    meds[[length(meds) + 1]] <- .rb_attach_facets(data.frame(
      pos = g$pos, y = lv$median, xmin = g$pos - max(widths) / 2,
      xmax = g$pos + max(widths) / 2), g)
    if (showfliers && length(lv$fliers)) {
      fliers[[length(fliers) + 1]] <- .rb_attach_facets(data.frame(pos = g$pos, y = lv$fliers), g)
    }
  }
  ...
  rect_aes <- if (vert) {
    ...
    ggplot2::aes(ymin = .data$xmin, ymax = .data$xmax, xmin = .data$ymin, xmax = .data$ymax)
  }
  p <- ggplot2::ggplot(box_df) +
    ggplot2::geom_rect(rect_aes, fill = box_df$fillcol, colour = lc, linewidth = lw)

  # Median line (across the widest box).
  med_df <- do.call(rbind, meds)
  med_aes <- if (vert) ggplot2::aes(x = .data$xmin, xend = .data$xmax, y = .data$y, yend = .data$y)
             else ggplot2::aes(y = .data$xmin, yend = .data$xmax, x = .data$y, xend = .data$y)
  p <- p + ggplot2::geom_segment(data = med_df, mapping = med_aes,
                                 colour = "#262626", linewidth = .rb_lw(1.5))

  if (length(fliers)) {
    fl_df <- do.call(rbind, fliers)
    fl_aes <- if (vert) ggplot2::aes(x = .data$pos, y = .data$y) else ggplot2::aes(x = .data$y, y = .data$pos)
    p <- p + ggplot2::geom_point(data = fl_df, mapping = fl_aes, shape = 1,
                                 colour = "#737373", size = 1.5, stroke = .rb_lw(1))
  }
...
swarmplot <- function(data = NULL, x = NULL, y = NULL, hue = NULL, order = NULL,
                      ...
                      palette = NULL, size = 5, edgecolor = NULL, linewidth = 0,
                      legend = "auto", .facet_vars = NULL, ...) {
  if (!requireNamespace("ggbeeswarm", quietly = TRUE)) {
    stop("swarmplot() requires the 'ggbeeswarm' package. Install it with install.packages('ggbeeswarm').")
  }
  s <- rb_cat_setup(data, x, y, hue, order, hue_order, orient, .facet_vars)
  vert <- s$orient == "v"
  if (s$has_hue) {
  ...
  sw_args <- list(pt_aes, size = rb_area_to_size(size^2), stroke = linewidth,
                  cex = 2.2, ...)
  if (isTRUE(dodge) && s$has_hue) sw_args$dodge.width <- 0.8
  if (!s$has_hue) sw_args$colour <- colors
