...
.rb_blend_rgb <- function(nodes, x) {
  k <- nrow(nodes)
  if (k == 1) {
    return(matrix(nodes, nrow = length(x), ncol = 3, byrow = TRUE))
  }
  pos <- x * (k - 1)
...
color_palette <- function(
  ...
  if (is.null(palette)) {
    ...
    if (palette %in% names(SEABORN_PALETTES)) {
      ...
      pal <- do.call(
        cubehelix_palette,
        c(
          list(n_colors = n_colors),
          .parse_cubehelix_args(palette),
      ...
      if (reverse) {
        color <- substr(color, 1, nchar(color) - 2)
      }
      pal <- light_palette(
        color,
        n_colors,
  ...
  if (inherits(pal, "reaborn_cmap")) {
    return(pal)
  }
  if (
    as_cmap &&
      (is.null(palette) ||
        palette %in% names(SEABORN_PALETTES) ||
  ...
  hex <- vapply(
    pal,
    function(c) {
      if (is.character(c) && grepl("^#[0-9A-Fa-f]{6}$", c)) {
        toupper(c)
      } else {
        rb_rgb_to_hex(rb_color_to_rgb(c))
      }
    },
    character(1)
  )
  ...
  if (!is.null(desat)) {
    hex <- vapply(hex, desaturate, character(1), prop = desat)
  }
...
hls_palette <- function(
  ...
  hues <- seq(0, 1, length.out = as.integer(n_colors) + 1)[
    -(as.integer(n_colors) + 1)
  ]
  hues <- hues + h
  hues <- hues %% 1
...
dark_palette <- function(
  ...
  if (as_cmap) {
    return(rb_make_cmap(
      .rb_build_lut256(.seq_palette_nodes(color, reverse, input, 15)),
      "dark"
    ))
  }
...
light_palette <- function(
  ...
  if (as_cmap) {
    return(rb_make_cmap(
      .rb_build_lut256(.seq_palette_nodes(color, reverse, input, 95)),
      "light"
    ))
  }
...
diverging_palette <- function(
  ...
  lut256 <- .rb_build_lut256(nodes)
  if (as_cmap) {
    return(rb_make_cmap(lut256, "diverging"))
  }
...
blend_palette <- function(
  ...
  nodes <- t(vapply(
    colors,
    function(c) .input_color_to_rgb(c, input),
    numeric(3)
  ))
  lut256 <- .rb_build_lut256(nodes)
  if (as_cmap) {
    return(rb_make_cmap(lut256, name = "blend"))
  }
...
mpl_palette <- function(name, n_colors = 6, as_cmap = FALSE) {
  if (name %in% names(MPL_QUAL_PALS)) {
    ...
    if (as_cmap) {
      return(rb_make_cmap(t(vapply(pal, rb_color_to_rgb, numeric(3))), name))
    }
    return(rep_len(toupper(pal), n_colors))
  }
  lut <- .reaborn_cmaps[[name]]
  if (is.null(lut)) {
    stop(sprintf("'%s' is not a bundled colormap name", name))
  }
  lut256 <- t(vapply(lut, rb_color_to_rgb, numeric(3))) # already 256 midpoint rows
  if (as_cmap) {
    return(rb_make_cmap(lut256, name))
  }
...
.parse_cubehelix_args <- function(argstr) {
  if (startsWith(argstr, "ch:")) {
    argstr <- substr(argstr, 4, nchar(argstr))
  }
  reverse <- FALSE
  ...
  for (item in kv) {
    ...
    key <- if (parts[1] %in% names(kwarg_map)) {
      kwarg_map[[parts[1]]]
    } else {
      parts[1]
    }
...
set_color_codes <- function(palette = "deep") {
  ...
  pal <- color_palette(
    if (base %in% names(SEABORN_PALETTES)) base else palette,
    10
  )
...
.input_color_to_rgb <- function(color, input = "rgb") {
  if (identical(input, "hls")) {
    return(.hls_to_rgb(color[1], color[2], color[3]))
  }
  if (identical(input, "husl")) {
    return(pmin(pmax(.husl_to_rgb(color[1], color[2], color[3]), 0), 1))
  }
