...
rb_assign_variables <- function(data = NULL, ...) {
  ...
  for (role in names(vars)) {
    v <- vars[[role]]
    if (
      is.character(v) && length(v) == 1L && !is.null(data) && v %in% names(data)
    ) {
      cols[[role]] <- data[[v]]
      names_map[[role]] <- v
      ...
      stop(sprintf(
        "Variable '%s' = '%s' given but no `data` to look it up in",
        role,
        v
      ))
    } else {
      cols[[role]] <- v
      names_map[[role]] <- if (!is.null(names(vars)) && nzchar(role)) {
        role
      } else {
        NULL
      }
  ...
  for (role in names(cols)) {
    if (length(cols[[role]]) == 1L && n > 1L) {
      cols[[role]] <- rep(cols[[role]], n)
    }
  }
  df <- if (length(cols)) {
    as.data.frame(
