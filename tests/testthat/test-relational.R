...
test_that("rb_categorical_order matches seaborn ordering rules", {
  ...
  expect_identical(rb_categorical_order(factor(c("b", "a"), levels = c("b", "a"))),
                   c("b", "a"))
  expect_identical(rb_categorical_order(c(3, 1, 2)), c("1", "2", "3"))
  expect_identical(rb_categorical_order(c("z", "a", "m")), c("z", "a", "m"))
  expect_identical(rb_categorical_order(c("z", "a"), order = c("a", "z")), c("a", "z"))
})
...
test_that("estimate aggregator: deterministic error methods are exact", {
  set.seed(1); vals <- rnorm(50, mean = 10, sd = 2)
  # se
  ...
  expect_equal(unname(a3[c("ymin", "ymax")]),
               unname(quantile(vals, c(0.05, 0.95), names = FALSE)))
})

test_that("bootstrap CI brackets the estimate", {
  set.seed(1); vals <- rnorm(100)
  a <- rb_estimate_aggregator(vals, "mean", list("ci", 95), n_boot = 500, seed = 0)
...
test_that("scatterplot builds a composable reaborn_plot", {
  ...
  p <- scatterplot(data = pen, x = "bill_length_mm", y = "bill_depth_mm", hue = "species")
...
test_that("relplot facets and returns a reaborn_plot", {
  ...
  p <- relplot(data = tips, x = "total_bill", y = "tip", hue = "smoker", col = "time")
  expect_s3_class(p, "reaborn_plot")
  expect_no_error(ggplot2::ggplot_build(p))
  pl <- relplot(data = load_dataset("fmri"), x = "timepoint", y = "signal",
