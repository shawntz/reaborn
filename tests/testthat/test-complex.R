...
test_that("violinplot builds for all inner kinds and orientations", {
  ...
  for (inner in c("box", "quart", "stick", "point")) {
    expect_no_error(ggplot2::ggplot_build(violinplot(data = tips, x = "day", y = "total_bill", inner = inner)))
  }
  expect_no_error(ggplot2::ggplot_build(violinplot(data = tips, x = "total_bill", y = "day")))
  expect_no_error(ggplot2::ggplot_build(violinplot(data = tips, x = "day", y = "total_bill", hue = "sex", split = TRUE)))
})
...
test_that("letter-value computation matches the seaborn algorithm", {
  ...
  expect_true(lv$values[1] < lv$median && lv$median < lv$values[length(lv$values)])
...
test_that("boxenplot builds", {
  ...
  expect_no_error(ggplot2::ggplot_build(boxenplot(data = pen, x = "species", y = "body_mass_g")))
  for (wm in c("exponential", "linear", "area")) {
    expect_no_error(ggplot2::ggplot_build(boxenplot(data = pen, x = "species", y = "body_mass_g", width_method = wm)))
  }
})
...
test_that("swarmplot builds (beeswarm)", {
  ...
  expect_no_error(ggplot2::ggplot_build(swarmplot(data = tips, x = "day", y = "total_bill")))
  expect_no_error(ggplot2::ggplot_build(swarmplot(data = tips, x = "day", y = "total_bill", hue = "sex", dodge = TRUE)))
})

test_that("jointplot and pairplot return patchwork compositions", {
  pen <- load_dataset("penguins")
  skip_if_not_installed("patchwork")
  j <- jointplot(data = pen, x = "bill_length_mm", y = "bill_depth_mm", hue = "species")
  expect_s3_class(j, "patchwork")
  pp <- pairplot(pen, vars = c("bill_length_mm", "bill_depth_mm"), hue = "species")
...
test_that("palplot and FacetGrid build; dogplot is harmless", {
  ...
  expect_no_error(ggplot2::ggplot_build(FacetGrid(load_dataset("tips"), col = "time")))
  expect_message(dogplot())
})

test_that("sns.* aliases exist for all M4 functions", {
  for (fn in c("sns.violinplot", "sns.boxenplot", "sns.swarmplot", "sns.jointplot",
               "sns.pairplot", "sns.clustermap", "sns.palplot", "sns.dogplot",
               "sns.FacetGrid")) {
